...
- Tooling ([06-tooling-and-ecosystem.md](06-tooling-and-ecosystem.md)): added `#[kabla]`/`#[baada]`
  ...
  `pata jaribu` row for the new `--muda <sekunde>` per-test timeout flag and the `pata thibitisha`
  row for its type-stability check against the most recent `v<semver>` git tag.
